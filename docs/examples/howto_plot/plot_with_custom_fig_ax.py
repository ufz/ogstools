...
# %%
# It is possible to plot various properties in different subplots of the
# same figure:
...
meshseries.read(0).plot_contourf(ot.properties.temperature, fig=fig, ax=ax[0])
meshseries.read(1).plot_contourf(ot.properties.temperature, fig=fig, ax=ax[1])
diff_mesh = meshseries.read(1).difference(
    meshseries.read(0), ot.properties.temperature
)
