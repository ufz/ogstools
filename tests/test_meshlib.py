...
class TestUtils:
    ...
    def test_all_types(self):
        ...
        pytest.raises(TypeError, MeshSeries, __file__)

        for mesh_series in [pvd, xdmf]:
            assert mesh_series.read(0) == mesh_series.read_closest(1e-6)

            assert not np.any(np.isnan(mesh_series.timesteps))
            assert not np.any(np.isnan(mesh_series.values("temperature")))

            assert mesh_series.timevalues[
                mesh_series.closest_timestep(1.0)
    ...
    def test_sample_over_polyline_multi_segment(self):
        ...
        assert not np.any(np.isnan(ms_sp[properties.temperature.data_name]))

        assert (
            np.abs(ms_sp[properties.temperature.data_name].to_numpy())
            > np.zeros_like(ms_sp[properties.temperature.data_name].to_numpy())
        ).all()
