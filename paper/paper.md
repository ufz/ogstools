...
Each sub-library either transforms OpenGeoSys specific data into common Python data structures (e.g. Pyvista, Pandas, Numpy, Matplotlib, Pint), or vice versa. User must be able to use just a subset of the library (no lock-in) also when they prefer to work with OpenGeoSys from command line and not with OGSTools.

### Reuse OGS command line tools

The new functionality must combine them to cover more complex tasks than any single tool can handle alone.

### Fail loudly

Silently producing wrong results is the highest risk in our simulation workflows. OGSTools therefore raises errors immediately when constraints or plausibility checks are violated, prioritizing early failure over silent pass-through.

### Large data awareness

Meshes can exceed one million cells. The top-level API is designed to be approachable for small datasets at entry level, while experts working with large data can tune performance via lower-level API access or fall back to command-line tools and custom code.

### Usability

OGSTools follows the principle that simple tasks must be easy and complex tasks must be possible — achieved by shifting inherent complexity from user code into this library. Recurring tasks such as mesh preparation, simulation setup, and result extraction are automated with sensible defaults, in deliberate contrast to OpenGeoSys which requires explicit configuration at every step. Type hints and consistent terminology with OpenGeoSys lower the cognitive barrier for new users. Complex workflows remain tractable because they can always be decomposed into simpler, independently accessible steps — each controllable by a single parameter or method call. To support broad adoption within the OGS user community, the library is deliberately integrated at key locations: the official OGS benchmarks, executable test cases, and contexts where previously used libraries were employed.

### Infrastructure

Code development is centralized on a self-hosted GitLab instance and maintained by the OGS core developer team. A multi-platform CI pipeline (Linux, Windows, macOS) enforces code quality via ruff, mypy, and black through pre-commit hooks, and reports test coverage on every merge request. The test suite uses pytest with markers to separate concerns — smoke, regression, performance, and maintenance tests — combining example-based and property-based testing (Hypothesis). Documentation is built with Sphinx using MyST Markdown for written content; examples are organized by task as plain Python scripts that are automatically converted to Jupyter notebooks, with Binder integration available for every released version. System tests with confidential, realistic datasets are maintained in a separate, access-controlled GitLab repository. To address the need for a versioned analysis environment [@Blomer2014], OGSTools ships a pinned dependency environment updated with every release, while also being continuously tested against the latest dependencies to catch breaking changes early.
...
Often the starting point of research.
