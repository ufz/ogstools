...
class TestSimplifiedMesh:
    ...
    def test_3_d_points_and_cells_of_coarse_to_fine_meshes(self):
        mesh_df_coarseZ = create.dataframe_from_csv(
            1,
            self.layerset,
            self.materialset,
            self.surfacedata,
        )
        layerset_coarse = create.LayerSet.from_pandas(mesh_df_coarseZ)
        ...
        mesh_df_fineZ = create.dataframe_from_csv(
            2,
            self.layerset,
            self.materialset,
            self.surfacedata,
        )
        layerset_fine = create.LayerSet.from_pandas(mesh_df_fineZ)
