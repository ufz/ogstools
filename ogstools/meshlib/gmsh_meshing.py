...
def gen_bhe_mesh(
    ...
    """
    Create a generic BHE mesh for the Heat_Transport_BHE-Process with additionally submeshes at the top, at the bottom and the groundwater inflow, which is exported in the OGS .msh format. Refinement layers are placed at the Top-Surface/BHE-begin, the groundwater end/start and the end of the BHE's. Currently only the same BHE begin depth for all BHE's is supported. See detailed description of the parameters below:
        ...
        if z_min_id == 3:  # and id_j==1 - not needed, but logically safer
            if id_j == id_j_max:
                ...
                space = (
                    np.abs(z_min - z_max)
                    - n_refinement_layers * target_z_size_fine
                )  # all are negative values, because of the extrusion in negative z direction -> z_min -- end Layer, z_max -- start layer
                if (
                    np.abs(space)
                    <= (n_refinement_layers + 1) * target_z_size_fine
                ):
            ...
            if (
                ...
                if (  # previous elif, one semantic block of different cases -> switch to if, because of ruff error
                    ...
                    if np.abs(groundwater[g][0]) == np.sum(
                        ...
                        icl = 1.2  # for all layers, which are top and bottom critical
                elif (
                    np.sum(layer[: start_groundwater + 1])
                    - np.abs(groundwater[g][0])
                    < n_refinement_layers * target_z_size_fine
    ...
    for i in range(0, len(layer)):  # Schleife zum Berechnen der Layer-Struktur
        ...
        absolute_height_of_layers: list[float] = []
        number_of_layers.append([])
    ...
    # define the inner square with BHE inside
    # compute the box size from the BHE-Coordinates
    x_min: float = np.min(BHE_array[:, 0]) - dist_box_x
    x_max: float = np.max(BHE_array[:, 0]) + dist_box_x
    y_min: float = np.min(BHE_array[:, 1]) - dist_box_y
    y_max: float = np.max(BHE_array[:, 1]) + dist_box_y
