...
To address `The Need for a Versioned Data Analysis Software Environment` \[@Blomer2014\] `OGSTools` provides additionally a pinned environment, updated at least once per release. While reproducibility requires environments with pinned dependencies. `OGSTools` is additionally tested with the latest dependencies to receive early warning of breaking changes and support long-term sustainability of the codebase.
