...
def gen_bhe_mesh(
        ...
        if z_min_id == 3:  # and id_j==1 - not needed, but logically safer
            if id_j == id_j_max:
                ...
                space = (
                    np.abs(z_min - z_max)
                    - n_refinement_layers * target_z_size_fine
                )  # alles negative Werte, da in negative z-Richtung extrudiert wird -> z_min -- Ende Layer, z_max -- Anfang layer
                if (
                    np.abs(space)
            ...
            if np.abs(BHE_array[j, 3]) < np.sum(layer[: i + 1]) and np.abs(
                ...
                if (
                    ...
                    if np.abs(BHE_array[j, 3]) == np.sum(
                        ...
                        BHE_to_soil[
                            j, 4
                        ] = 1.2  # für Schichten die sowohl top als auch bottom critical sind
                elif (
                    np.sum(layer[: i + 1]) - np.abs(BHE_array[j, 3])
                    < n_refinement_layers * target_z_size_fine
                ):
    ...
    for i in range(0, len(layer)):  # Schleife zum Berechnen der Layer-Struktur
        list_of_needed_depths = needed_depths[
            i
        ]  # alle Tiefen, die einen Knoten im Netz brauchen
    ...
    all_extrusion = np.array(
        [
            flatten_concatenation(All_extrusion_depths),
            flatten_concatenation(All_extrusion_layers),
        ]
    ).transpose()
