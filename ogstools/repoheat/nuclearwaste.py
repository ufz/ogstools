...
class NuclearWaste:
    ...
    time_deposit: Union[PlainQuantity, float]
    ...
    def heat(
        self, t: Numeric, baseline: bool = False, ncl_id: Opt[int] = None
    ) -> Numeric:
        """Calculate the heat of a nuclear waste proxy model.

        :param t:           Time or timesteps at which to calculate the heat.
        ...
        _t = Q_(t).magnitude
        _nuclide_powers = Q_(self.nuclide_powers, units.power).magnitude
        _decay_consts = Q_(self.decay_consts, f"1/{units.time}").magnitude
        _num_bundles = self.num_bundles
        _time_interim = Q_(self.time_interim, units.time).magnitude
        _time_deposit = Q_(self.time_deposit, units.time).magnitude
        if baseline:
...
class Repo:
    ...
    def heat(
        ...
        """Calculate the heat produced by the repo at time t.

        :param t:           Time at which the heat is calculated.
        :param ncl_id:      If given, only output the heat by nuclide with this
                            id, else sum the heat over all nuclides.
        :param baseline:    If True, evaluate one bundle for each waste with no
                            interim or deposition time delay.
        :returns:           Heat generated by the repo at time t.
        """
