...
# %%
mesh_series = examples.load_meshseries_THM_2D_PVD(spatial_unit="km")
...
# %% [markdown]
# Difference between two meshseries
# ----------------------------------
# It is possible to calculate the difference between two MeshSeries objects.
# First, an artificial data set will be generated by copying an example data set and increasing the pressure values by 25%.

# %%
ms = examples.load_meshseries_THM_2D_PVD(spatial_unit="km", time_unit="d")
ms2 = ms.copy()
ms2.point_data["pressure"] *= 1.25

# %% [markdown]
# Now a difference between original and altered MeshSeries can be computed:

# %%
diff_ms = ot.MeshSeries.difference(ms2, ms, ot.variables.pressure)

# %% [markdown]
# As a reference, the last steps of the original and altered MeshSeries
# can be plotted:
...
# same way as any other one. However, to plot the pressure difference either
# use the string "pressure_difference" (see comment in the code block below) or the derived variable.
